...
/** "Modules" landing section — the full PRD module grid. */
export function ModulesSection() {
  const { lang } = useLanguage();
  const hero = pick(lang, HERO);

  return (
    <section id="modules" className="mx-auto w-[min(1080px,90%)] scroll-mt-28 pb-14 pt-14">
        ...
        {MODULES.map((moduleItem) => {
          ...
          return (
            <div key={moduleItem.code} className="card p-6">
              ...
              <h3 className="font-display mt-4 text-base font-bold text-[var(--text-primary)]">
                {content.title}
              </h3>
